...
**설계 원칙:** `azure-functions-langgraph`는 LangGraph 런타임 노출을 담당합니다. `azure-functions-validation`은 요청/응답 검증과 직렬화를 담당합니다. `azure-functions-openapi`는 API 문서화와 스펙 생성을 담당합니다.
