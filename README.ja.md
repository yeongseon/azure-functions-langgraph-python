...
シーケンス／状態遷移図（Invoke・バッファリング Stream とオーバーフロー経路、スレッドライフサイクルのステートマシン、Blob リースのライフサイクル）は、正式な英語ドキュメントでレンダリングされます：

- [Architecture](docs/architecture.md) — モジュールフローチャート、Invoke/Stream シーケンス（オーバーフロー含む）、スレッドライフサイクルのステートマシン
- [Production Guide](docs/production-guide.md#distributed-thread-locking) — スレッドライフサイクルおよび分散リースライフサイクル図
...
**設計原則:** `azure-functions-langgraph` は LangGraph ランタイムの公開を担当します。 `azure-functions-validation` はリクエスト/レスポンスのバリデーションとシリアライズを担当します。 `azure-functions-openapi` は API ドキュメントとスペック生成を担当します。
...
| **azure-functions-langgraph-python** | Azure Functions 向け LangGraph デプロイアダプター |
